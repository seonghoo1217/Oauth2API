...
public class OAuthService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		UserInfo userProfile = OAuthAttributes.extract(registrationId, attributes); // registrationId에 따라 유저 정보를 통해 공통된 UserProfile 객체로 만들어 줌
	...
	private Member saveOrUpdate(UserInfo userProfile) {
		Optional<Member> member = memberRepository.findByOauthId(userProfile.getOauthId());
		if (member.isPresent()){
